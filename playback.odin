...
play_song :: proc() {
    ...
    spath := strings.clone_to_cstring(library[curr_idx])
    ...
    if res != .SUCCESS {
        fmt.eprintfln("error loading sound file %s: %v", curr_track, res)
        return
    }
...
play_pause :: proc() {
    ...
    } else if ma.sound_at_end(&curr_track) {
        play_next()
        return
    }
